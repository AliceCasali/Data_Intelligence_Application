...
class UCB(Learner):
    ...
    def pull_arm_per_class(self):
        ...
        for i in range(self.n_classes):
            uppers.append(self.empirical_means[i,:] + self.confidence[i,:])
    ...
    def update(self, pulled_arms, reward, to_be_updated):
        ...
        for c in range(self.n_classes):
            ...
            self.empirical_means[c][pulled_arms[c]] = (self.empirical_means[c][pulled_arms[c]]*(self.t-1) + reward[c])/self.t
            for a in range(self.n_arms):
                n_samples = len(self.rewards_per_arm[c])
                self.confidence[c][a] = (2*np.log(self.t)/n_samples)**0.5 if n_samples > 0 else np.inf
